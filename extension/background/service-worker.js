...
function hasSuggestionFixReady(suggestion) {
  const inlineFix = String(suggestion?.after || suggestion?.fix?.replacement || "").trim();
  const prefetchedCode = String(suggestion?.prefetched_fix?.fixed_code || "").trim();
  return Boolean(inlineFix || prefetchedCode);
}

function shouldPrefetchFix(snapshot, suggestion) {
  const language = String(snapshot?.language || "").toLowerCase();
  if (language !== "python") {
    return false;
  }

  const ruleId = String(suggestion?.rule_id || "");
  if (!ruleId) {
    return false;
  }

  const disallowedRules = new Set(["info.no-issues", "analysis.error"]);
  return !disallowedRules.has(ruleId);
}

async function prefetchFixesForSuggestions(snapshot, normalizedResult, settings) {
  const suggestions = Array.isArray(normalizedResult?.suggestions) ? normalizedResult.suggestions : [];
  ...
  for (const suggestion of suggestions) {
    ...
    attempted += 1;
    try {
      const fixResult = await requestValidatedFix({
        code: snapshot.code,
        language: snapshot.language || "python",
        suggestion,
        exec_timeout_s: 2,
      ...
      const fixedCode = String(fixResult?.fixed_code || "");
      if (fixResult?.applied && fixedCode && fixedCode !== snapshot.code) {
        hydrated.push({
          ...suggestion,
          prefetched_fix: {
            fixed_code: fixedCode,
            message: String(fixResult?.message || "Validated fix generated by backend."),
  ...
  return {
    ...
    metadata: {
      ...
      fix_prefetch_attempted: attempted,
      fix_prefetch_succeeded: prefetched
    }
  };
}
...
async function analyzeSnapshot(tabId, snapshot) {
  ...
  try {
    ...
    const hydratedResult = await prefetchFixesForSuggestions(snapshot, normalizedResult, settings);
    ...
    const msg = { type: "ANALYSIS_RESULT", payload: hydratedResult };
