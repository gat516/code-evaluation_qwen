...
def _apply_ai_python_fix(code: str, suggestion: Suggestion, timeout_s: int | None) -> tuple[str, str]:
    ...
    if not fixed_code.strip():
        return code, "AI fix generation returned empty code."
...
def fix(payload: FixRequest, _: None = Depends(verify_api_key)) -> FixResponse:
    ...
    if not applied:
        fallback_code = _apply_python_fix(payload.code, payload.suggestion)
        fallback_validation = _validate_python_fix(payload.code, fallback_code, payload.exec_timeout_s)
        fallback_applied = bool(fallback_validation.get("syntax_ok") and fallback_validation.get("changed"))
        if fallback_applied:
            fixed_code = fallback_code
            validation = fallback_validation
            applied = True
            message = "Validated fix generated by deterministic fallback after AI attempt."
        else:
            message = f"{ai_message} {fallback_validation.get('reason') or 'No valid fix generated.'}".strip()
    else:
